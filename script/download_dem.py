...
# Bounding box dell’Italia (min_lon, max_lon, min_lat, max_lat)
italy_bbox = {
    "min_x": 6.7499552751,
    "max_x": 18.4802470232,
    "min_y": 36.619987291,
...
# Dimensione della tessera (in gradi)
tile_width = 0.5
tile_height = 0.5

# Cartella dove salvare i tile DEM
tiles_directory = "italy_tiles"
os.makedirs(tiles_directory, exist_ok=True)

# Calcola il numero di tile in orizzontale e verticale
num_tiles_x = int(np.ceil((italy_bbox["max_x"] - italy_bbox["min_x"]) / tile_width))
num_tiles_y = int(np.ceil((italy_bbox["max_y"] - italy_bbox["min_y"]) / tile_height))

def refresh_token():
    """Richiede un nuovo access token se necessario."""
    global oauth
    print("Rinnovo del token...")
    oauth.fetch_token(
        token_url="https://identity.dataspace.copernicus.eu/auth/realms/CDSE/protocol/openid-connect/token",
...
# Evalscript per la richiesta DEM (riduce l’elevazione in km)
evalscript = """
//VERSION=3
function setup() {
  return {
...
def download_tile(tile_x, tile_y):
    """Scarica un singolo tile DEM dato l’indice di riga e colonna."""
    try:
        token_info = oauth.token
        if not token_info or "expires_at" not in token_info or token_info["expires_at"] < time.time():
            refresh_token()
    ...
    # Calcola le coordinate del tile corrente
    tile_min_x = italy_bbox["min_x"] + tile_x * tile_width
    tile_max_x = min(tile_min_x + tile_width, italy_bbox["max_x"])
    tile_min_y = italy_bbox["min_y"] + tile_y * tile_height
    tile_max_y = min(tile_min_y + tile_height, italy_bbox["max_y"])
...
def process_dem():
    """
    Scarica tutte le tessere DEM dell’Italia e le unisce in un unico file GeoTIFF.
    """
    all_filenames = []
    for tile_x in range(num_tiles_x):
        for tile_y in range(num_tiles_y):
    ...
    if all_filenames:
        # Percorso di output per il DEM combinato
        combined_filename = os.path.join(os.getcwd(), "data/italy_dem_combined.tiff")
        gdal_command = f"gdal_merge.py -o {combined_filename} {' '.join(all_filenames)}"
        print(f"Esecuzione del comando GDAL: {gdal_command}")
        subprocess.run(gdal_command, shell=True)
        print(f"Tutti i tile sono stati combinati in: {combined_filename}")

        # Elimina i tile temporanei
        for filename in all_filenames:
            try:
                os.remove(filename)
                print(f"File temporaneo eliminato: {filename}")
